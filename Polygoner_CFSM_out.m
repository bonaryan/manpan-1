...
for i = [1:matrix_size(1)];
    for j = [1:matrix_size(2)];
        for k= [1:matrix_size(3)];
            ...
            node = [(1:length(profiles{i, j, k}))', profiles{i, j, k}(1, :)', profiles{i, j, k}(2, :)', ones(length(profiles{i, j, k})', 1), ones(length(profiles{i, j, k})', 1), ones(length(profiles{i, j, k})', 1), ones(length(profiles{i, j, k})', 1), 100*ones(length(profiles{i, j, k})', 1)];
            elem = [(1:length(profiles{i, j, k})-1)', (1:length(profiles{i, j, k})-1)', (2:length(profiles{i, j, k}))', 0.1*ones(length(profiles{i, j, k})-1', 1), 100*ones(length(profiles{i, j, k})-1', 1)];
            prop = [100, Ex, Ey, vx, vy, G ];
